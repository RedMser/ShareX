        ...
        public byte[] EncodeLossless(Bitmap bmp, int speed, bool info = false)
        {
            ...
            try
            {
                ...
                if (info)
                {
                    stats = (WebPAuxStats)Marshal.PtrToStructure(ptrStats, typeof(WebPAuxStats));
                    string features = "";
                    if ((stats.lossless_features & 1) > 0)
                        features = " PREDICTION";
                    if ((stats.lossless_features & 2) > 0)
                        features = features + " CROSS-COLOR-TRANSFORM";
                    if ((stats.lossless_features & 4) > 0)
                        features = features + " SUBTRACT-GREEN";
                    if ((stats.lossless_features & 8) > 0)
                        features = features + " PALETTE";
                    MessageBox.Show("Dimension: " + wpic.width + " x " + wpic.height + " pixels\n" +
                                    "Output:    " + stats.coded_size + " bytes\n" +
